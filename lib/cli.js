...
const tableOptions = {
    ...
    head: ['app name', 'status', 'port', 'restarts', 'uptime', 'memory', 'cpu%'].map(function(field)
    {
        return field.cyan;
    })
};
...
const cli = {
    ...
    rm: function(arg1, arg2)
    {
        ...
        rl.question('POD'.green + ' | really remove ' + appname.cyan + '? (y/N): ', function(reply)
        {
            ...
            {
                log('cancelled deleting ' + appname.cyan);
                process.exit(0);
            }
        });
    },
...
if(!fs.existsSync(globalConfigPath))
{
    ...
    console.log(
        ...
        'Please specify a directory for pod to put files in.\n' +
...
function parseCommand()
{
    ...
    else
    {
        if(command)
        {
            warn('unknown command ' + command.cyan);
...
function toArray(app)
{
    ...
    else if(restarts > 0 && restarts < 10)
    {
    ...
    else if(status === 'BROKEN' || status === 'ERROR')
    {
        status = status.red;
    }

    return [
        toCLITableObject(app.name.cyan),
        toCLITableObject(status),
        toCLITableObject(app.port || '--'),
        toCLITableObject(restarts || '--'),
        toCLITableObject(app.uptime || '--'),
        toCLITableObject(app.memory || '--'),
        toCLITableObject(app.cpu || '--')
    ];
}

function toCLITableObject(text)
{
    const obj = {
        hAlign: 'center',
    };
